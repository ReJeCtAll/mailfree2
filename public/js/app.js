...
function showHeaderLoading(t) { if (els.listLoading) { els.listLoading.innerHTML = `<span class="spinner"></span>${t || window.__('common.loading')}`; els.listLoading.style.display = 'flex'; }}
function hideHeaderLoading() { if (els.listLoading) els.listLoading.style.display = 'none'; }
function showCountdown() { if (els.listLoading) { els.listLoading.innerHTML = `<span class="countdown-icon">⏱</span>${window.__('common.countdown', { count: countdown })}`; els.listLoading.style.display = 'flex'; }}
...
async function refresh() {
  ...
  if (!mailbox) return;
  try {
    showHeaderLoading(isFirstLoad() ? window.__('common.loading') : window.__('common.updating'));
...
async function loadMailboxes(opts = {}) {
  ...
  try {
    ...
    try { const q = document.getElementById('quota'); if (q) q.textContent = window.__('mailbox.count', { total: total }); } catch(_) {}
  } catch(_) {}
...
if (els.tabSent) els.tabSent.onclick = () => { setView(true); els.tabSent.classList.add('active'); els.tabInbox?.classList.remove('active'); if (els.boxTitle) els.boxTitle.textContent = window.__('email.sentbox'); if (els.boxIcon) els.boxIcon.textContent = '📤'; resetPager(els); refresh(); };
...
(async () => {
  ...
  try { const qr = await api('/api/user/quota'); const q = await qr.json(); const el = document.getElementById('quota'); if (el && q) { el.textContent = isAdmin() ? window.__('mailbox.count', { total: q.total || 0 }) : `${q.used || 0} / ${q.limit || 0}`; }} catch(_) {}
  await loadMailboxes();
