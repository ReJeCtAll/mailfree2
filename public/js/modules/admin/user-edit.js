...
export function validateEditForm(data, isNew = false) {
  if (isNew) {
    if (!data.username || !data.username.trim()) {
      return { valid: false, error: window.__('login.empty.username') };
    }
    if (!data.password || !data.password.trim()) {
      return { valid: false, error: window.__('login.empty.password') };
    }
  }
  
  if (data.mailboxLimit !== undefined && (isNaN(data.mailboxLimit) || data.mailboxLimit < 0)) {
    return { valid: false, error: window.__('user.mailbox.limit') };
  }
...
export function createUserModalContent() {
  return `
    <div class="form-group">
      <label for="new-username">${window.__('user.enter.username')}</label>
      <input type="text" id="new-username" placeholder="${window.__('user.enter.username')}" autocomplete="off">
    </div>
    <div class="form-group">
      <label for="new-password">${window.__('login.empty.password')}</label>
      <input type="password" id="new-password" placeholder="${window.__('login.empty.password')}" autocomplete="new-password">
    </div>
    <div class="form-group">
      <label for="new-role">${window.__('role.admin.user')}</label>
      <select id="new-role">
        <option value="user">${window.__('role.regular.user')}</option>
        <option value="admin">${window.__('role.admin.user')}</option>
      </select>
    </div>
    <div class="form-group">
      <label for="new-limit">${window.__('user.mailbox.limit')}</label>
