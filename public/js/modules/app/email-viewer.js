...
export async function showEmailDetail(id, elements, api, showToast) {
  ...
  try {
    ...
    modalSubject.innerHTML = `<span class="modal-icon">📧</span><span>${escapeHtml(email.subject || window.__('common.no.subject'))}</span>`;
...
export async function deleteEmailById(id, api, showToast, showConfirm, refresh) {
  const confirmed = await showConfirm(window.__('email.confirm.delete'));
  if (!confirmed) return;
  
  try {
    const r = await api(`/api/email/${id}`, { method: 'DELETE' });
    if (r.ok) {
      showToast(window.__('email.deleted'), 'success');
      await refresh();
    }
  } catch(e) {
...
export async function deleteSentById(id, api, showToast, showConfirm, refresh) {
  const confirmed = await showConfirm(window.__('email.confirm.delete.sent'));
  if (!confirmed) return;
  
  try {
    const r = await api(`/api/sent/${id}`, { method: 'DELETE' });
    if (r.ok) {
      showToast(window.__('email.sent.deleted'), 'success');
      await refresh();
    }
  } catch(e) {
    showToast(e.message || window.__('common.delete.fail'), 'error');
...
export async function copyFromEmailList(event, id, api, showToast) {
  ...
  if (code) {
    try {
      ...
      showToast(window.__('email.code.copied.msg', { code: code }), 'success');
    } catch(_) {
    ...
    try {
      ...
      showToast(window.__('email.content.copied'), 'success');
    } catch(_) {
      showToast(window.__('common.copy.fail'), 'error');
    }
  }
